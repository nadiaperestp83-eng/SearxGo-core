...
void BraveBrowserView::UpdateFocusModeState() {
  ...
  if (enabled) {
    // For "insecure" (e.g. http, cert error) pages, configure the browser view
    // as if Focus Mode is not enabled.
    if (auto* location_bar = GetLocationBar()) {
      if (auto* model = location_bar->GetLocationBarModel()) {
        switch (model->GetSecurityLevel()) {
